<template>
  <div class="modal-bg" v-if="showDetailsModal" @click.self="showDetailsModal = false" style="z-index:1100">
    <div class="modal" style="max-width:680px;padding:0;overflow:hidden">
      ...
      <div v-if="detailRecord" style="padding:20px 24px;max-height:70vh;overflow-y:auto">
        ...
        <!-- Release Date (shown when Released) -->
        <div v-if="detailRecord.status === 'Released'" style="background:rgba(39,174,96,0.07);border:1px solid rgba(39,174,96,0.25);border-radius:10px;padding:12px 16px;margin-bottom:14px;display:flex;align-items:center;gap:10px">
          <span style="font-size:18px">🔓</span>
          <div>
            <div style="font-size:10px;font-weight:700;color:#27ae60;letter-spacing:1px;text-transform:uppercase">Release Date</div>
            <div style="font-size:14px;color:rgba(255,255,255,0.9);font-weight:600;margin-top:2px">
              {{ detailRecord.releaseDate ? fmtDate(detailRecord.releaseDate) : '—' }}
            </div>
          </div>
        </div>
      ...
      <div style="padding:14px 24px;display:flex;gap:10px;flex-wrap:wrap;border-top:1px solid var(--border2)">
        ...
        <button class="btn btn-pdf" @click="doSavePDF" :disabled="pdfSaving">
          <svg v-if="!pdfSaving" viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="15" y2="15"/></svg>
          <svg v-else viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" style="animation:spin 1s linear infinite"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
          {{ pdfSaving ? 'Generating...' : 'Save PDF' }}
        </button>
  ...
  <div v-if="detailRecord" id="details-receipt-print" class="details-print-area">
    <div class="receipt-preview" style="padding: 0 60px;">
...
async function doSavePDF() {
  ...
  try {
    ...
    const el = document.getElementById('details-receipt-print')
    if (!el) return
    ...
    const plate = detailRecord.value?.plate || 'record'
    const date = detailRecord.value?.date || new Date().toISOString().split('T')[0]
    pdf.save(`impound_${plate}_${date}.pdf`)
  } catch (e) {
